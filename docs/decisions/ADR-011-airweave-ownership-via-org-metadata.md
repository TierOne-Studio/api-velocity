...
**Status:** Accepted (Decision 3 + Decision 4 amended 2026-05-23 after security review; OAuth-transport client contract amended 2026-05-25; body-level `organizationId` on collection create amended 2026-05-27; "Airweave Collections" wire/config field rename amended 2026-06-16)
**Date:** 2026-05-23
...
> **Amendment 6 (2026-06-16) — "Airweave Collections" rebrand: wire + persisted-config field rename.** The feature is branded **"Airweave Collections"** end-to-end. The backend's namespace/RBAC/DB-enum/metadata were already `airweave`-named, but a set of bare `collection*` identifiers (wire JSON fields, persisted `config` keys, internal symbols, UI strings) were inconsistent. This amendment renames them to the `airweaveCollection*` form. It is a **breaking wire change** coordinated FE+BE (see `docs/airweave-collections-rename-coordination-plan.md`).
>
> **What renames:**
> - **Wire JSON fields:** `AirweaveSourceConnectionSummary.collectionReadableId` (response) → `airweaveCollectionReadableId`; the delete-conflict `ConflictException` body field `collectionReadableId` → `airweaveCollectionReadableId`; the connect/session request body field `collectionId` → `airweaveCollectionId` (and the paired `@RequireAirweaveOwnershipFromBody('airweaveCollectionId')` decorator arg at `airweave.controller.ts`); the DELETE-collection response field `collectionId` → `airweaveCollectionId`.
> - **Persisted config keys:** `project_data_source.config.{collectionReadableId,collectionName}` → `{airweaveCollectionReadableId,airweaveCollectionName}`, migrated forward by `projects_005_rename_airweave_config_keys` (single idempotent `jsonb_set`+strip UPDATE, scoped to `kind='airweave_collection'` rows; integration-tested vs real Postgres across both-keys / partial / empty / non-airweave / idempotent variants). Historical migrations 002/003 are immutable and intentionally keep WRITING the old keys; 005 runs after them and renames forward — the idempotency read at `projects.migration.ts:167` stays on the old key by design.
> - **Internal symbols:** bare DTOs (`CreateCollectionBody`→`CreateAirweaveCollectionBody`, `SearchCollectionBody`→`SearchAirweaveCollectionBody`), local vars/params/props across both repos (incl. the admin/organizations allowlist-method params and the `AirweaveOwnershipGuard` internal var). **Terse method names on `Airweave*` classes (`createCollection`, `listCollections`, …) and the FE free service functions are intentionally KEPT** (the class/module already carries "Airweave"; renaming would stutter — KISS).
>
> **What does NOT change (already `airweave`, or external):** the route path `/api/airweave/collections` (renaming would stutter to `/airweave/airweave-collections`); the `project_data_source.kind = 'airweave_collection'` DB enum value + its CHECK constraint; the RBAC resource `airweave:*`; the org-metadata key `allowedAirweaveCollectionIds`; the route param `:collectionId` (+ `@Param('collectionId')` + the six param-based `@RequireAirweaveOwnership('collectionId')`); the Airweave SDK surface (`client.collections.*`, `readable_collection_id`, `readable_id`).
>
> **⚠️ Observability-contract impact (SRE action required).** Three structured-log keys rename via the var-rename cascade and are parsed by SRE aggregators (Decision 4 read-lockdown soak + audit signals): the read-lockdown warning `airweave.read_would_403` JSON key `collectionReadableId` → `airweaveCollectionReadableId` (`airweave-ownership.guard.ts`), and the `airweave.source_connection.created` / `airweave.source_connection.deleted` event keys `collectionReadableId` → `airweaveCollectionReadableId` (`airweave.service.ts`). **Update SRE dashboards/alerts that key off `collectionReadableId` for these events.** (If log-key stability is required, the keys can be pinned by decoupling the log shorthand from the renamed var with an explicit `collectionReadableId:` mapping — deliberately not done here, per the "rename everything" directive.)
>
> **Deploy:** breaking; FE+BE ship together. The migration runs at boot via `OnModuleInit`. If atomic deploy is impossible, a temporary BE dual-field read is the fallback (out of scope here).
