...
description: Use BEFORE writing the failing test in tdd-workflow Step 1, on any non-trivial code change. Forces explicit enumeration of failure modes (null, empty, large, race, partial, network, malformed, boundary) BEFORE deciding which tests to write. NOT for trivial single-line edits, type-only changes, documentation, or non-code work.
---
