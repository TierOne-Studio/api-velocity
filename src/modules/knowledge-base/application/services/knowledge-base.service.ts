...
export class KnowledgeBaseService {
  ...
  async list(scope: CallerScope): Promise<KnowledgeBase[]> {
    const orgId = this.requireOrg(scope);
    const rows = await this.repository.listForOrganization(orgId);
    return rows.map(toPublic);
  }

  async getById(scope: CallerScope, id: string): Promise<KnowledgeBase> {
    const orgId = this.requireOrg(scope);
...
function toPublic(row: KnowledgeBaseRow): KnowledgeBase {
  return {
    id: row.id,
