...
describe('KnowledgeBaseMigrationService', () => {
  let db: jest.Mocked<DatabaseService>;
  let service: KnowledgeBaseMigrationService;
  ...
  beforeEach(() => {
    ...
    service = new KnowledgeBaseMigrationService(db);
  ...
  it('runs the migration on a fresh database', async () => {
    ...
    expect(db.recordMigration).toHaveBeenCalledWith(
      'knowledge_base_001_create_org_knowledge_base',
    );
  });
  ...
  it('createTable emits CREATE TABLE with status CHECK constraint', async () => {
    ...
    expect(db.query).toHaveBeenCalledWith(
      expect.stringMatching(
        /CREATE TABLE IF NOT EXISTS org_knowledge_base/i,
      ),
    );
    expect(db.query).toHaveBeenCalledWith(
      expect.stringMatching(
  ...
  it('createTable creates the unique name index', async () => {
    ...
    expect(calls.some((sql) => /idx_org_kb_name/.test(sql))).toBe(true);
  });
});
